...
const MessagesPage = () => {
  ...
  const filteredMessages = rawMessages.filter((msg) => {
    ...
    if (!q) return true;
    return msg.sender.toLowerCase().includes(q) || msg.preview.toLowerCase().includes(q);
  });
  ...
  return (
    <div className="bg-white dark:bg-slate-900 text-gray-800 dark:text-slate-100 p-4 rounded-md flex-1 m-4 mt-0 border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <TableSearch value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search messages..." />
      </div>
      <div className="flex flex-col gap-4 mt-8">
        {filteredMessages.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-slate-400 py-8 text-center">No messages found matching &quot;{searchTerm}&quot;</p>
        ) : (
