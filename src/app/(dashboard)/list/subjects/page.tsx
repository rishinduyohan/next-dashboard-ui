...
type Subject = { id: number; name: string; teachers: string[] | string };
...
const SubjectListPage = () => {
  ...
  const renderRow = (item: Subject) => {
    const teachersDisplay = Array.isArray(item.teachers) ? item.teachers.join(", ") : item.teachers;
    return (
      <tr key={item.id} className="border-b border-gray-200 dark:border-slate-800 even:bg-slate-50 dark:even:bg-slate-800/40 text-sm hover:bg-RishlightSky dark:hover:bg-slate-800 transition-colors text-gray-700 dark:text-slate-200">
        ...
        <td className="hidden md:table-cell">{teachersDisplay}</td>
        {canEdit && (
          <td>
  ...
  const filteredData = (data.subject || []).filter((s: Subject) => {
    const q = searchTerm.toLowerCase().trim();
    if (!q) return true;
    const teachers = Array.isArray(s.teachers) ? s.teachers.join(" ").toLowerCase() : (s.teachers?.toLowerCase() || "");
    return (s.name?.toLowerCase() || "").includes(q) || teachers.includes(q);
  });
    ...
    <div className="bg-white dark:bg-slate-900 text-gray-800 dark:text-slate-100 p-4 rounded-md flex-1 m-4 mt-0 border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSearch value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search subjects..." />
          <div className="flex items-center gap-4 self-end">
            ...
            {canEdit && <FormModal table="subject" type="create" />}
          </div>
        </div>
      </div>
